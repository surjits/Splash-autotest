...
public class UserGroupPage extends BasePage {
    ...
    public String createUserGroup() throws Exception {
        usergroup_name = Utility.getRandomNumber("DPUSERGRP");
    ...
    public boolean isUserPresentInGroup(String group) {
        ...
        try {
           ...
           while(!isElementDisplayed(USERLIST_IN_GROUP)){
            ...
            test.log(LogStatus.INFO, "Snapshot Below: " + test.addScreenCapture(addScreenshot()));
            test.log(LogStatus.FAIL, "User not found", "User" + " " + "not present in user group");
        }
        return isPresent;
    }

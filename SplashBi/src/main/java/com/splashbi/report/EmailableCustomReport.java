...
public class EmailableCustomReport implements IReporter {
	
	private static final String emailableReportTemplateFile = Constant.CUSTOM_TESTNG_REPORT_PATH;
	...
	public void generateReport(List<XmlSuite> xmlSuites, List<ISuite> suites, String outputDirectory) 
	{
		try {
			...
			Date d = new Date();
			String fileName = d.toString().replace(":", "_").replace(" ", "_")+"_"+ "custom-emailable-report.html";
		...
		try {
			...
			for (ISuite tempSuite : suites) {
				...
				for (ISuiteResult result : testResults.values()) {
					 ...
					 * retBuf.append(this.stringArrayToString(testObj.
					 * getIncludedGroups())); retBuf.append("</td>");
					 * 
					 * // Exclude groups. retBuf.append("<td>");
					 * retBuf.append(this.stringArrayToString(testObj.
					 * getExcludedGroups())); retBuf.append("</td>");
					 */
					retBuf.append("</tr>");
				}
			}
